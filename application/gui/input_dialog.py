...
class OverwriteFileDialog(OverwriteDialog):
	...
	def _create_buttons(self):
		...
		self.button_replace = gtk.Button(label=_('Replace'))
		self.button_replace.set_can_default(True)

		OverwriteDialog._create_buttons(self)
		self._dialog.add_action_widget(self.button_replace, gtk.RESPONSE_YES)

		self._dialog.set_default_response(gtk.RESPONSE_YES)

	def _rename_button(self, entry):
		if entry.get_text() == self._rename_value:
			self.button_replace.set_label(_('Replace'))
		else:
			self.button_replace.set_label(_('Copy'))
