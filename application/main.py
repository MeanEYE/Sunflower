...
class Sunflower(Gtk.Application):
	...
	def __init__(self):
		...
		Gtk.Application.__init__(self, application_id=application_id, flags=Gio.ApplicationFlags.HANDLES_COMMAND_LINE)
		...
		option_entries = [version_entry, no_plugins_entry, no_load_tabs_entry, left_tab_entry, right_tab_entry,
						  left_terminal_entry, right_terminal_entry]
	...
	def do_activate(self):
		...
		if not self.window:
			self.window = MainWindow(application=self, dont_load_plugins=self.arguments is not None and self.arguments.dont_load_plugins)

		self.window.create_tabs(self.arguments)

	def do_command_line(self, command_line):
		"""Handle command line - called in the primary instances - unless multiple instances are allowed - then they all are primary"""

		def make_absolute_path(cwd, path):
			if '://' not in path:
		...
		if options.contains('left-tab'):
			self.arguments.left_tabs =  map(lambda path: make_absolute_path(command_line.get_cwd(), path), options.lookup_value('left-tab'))

		if options.contains('right-tab'):
			self.arguments.right_tabs = map(lambda path: make_absolute_path(command_line.get_cwd(), path), options.lookup_value('right-tab'))

		if options.contains('left-terminal'):
			self.arguments.left_terminals = map(lambda path: make_absolute_path(command_line.get_cwd(), path), options.lookup_value('left-terminal'))

		if options.contains('right-terminal'):
			self.arguments.right_terminals = map(lambda path: make_absolute_path(command_line.get_cwd(), path), options.lookup_value('right-terminal'))
	...
	def do_handle_local_options(self, options):
		"""Handle local command line options - if any application will terminate afterwards"""
